...
対象原稿: 「何を統制すべきか――候補因果グラフで読み解く多元宇宙分析」v0.8(2026-09-17; 旧題「回顧的な中学成績を統制すべきか…」)。
...
(固定版はタグ `j1-v0.8`; 公開・タグ付けの手順は `github/make_replication_archives.sh` と `github/publish_replication.sh`、
クリーンコピーでの合成データ再実行の記録は同アーカイブの `RELEASE_CHECK.md`)。

## 1. データと利用条件 / Data and access
- 入力: 東大社研・若年壮年パネル調査(JLPS-Y/M 2007 開始; 2011 追加標本; 2019 リフレッシュ標本 JLPS-2019)の **SSJDA 提供版個票**(w1–w19 統合 Stata ファイル、8,146 行 × 11,303 列)。本稿が使う変数は下記 2 節の 34 列のみ。
- 個票は SSJDA の利用条件により **ライセンスを受けた著者のローカル機でのみ**扱う。個票・個票由来の中間ファイルはこのアーカイブにも公開リポジトリにも含めない。集計出力は **セル N < 10 を秘匿**したうえで `results/` に置く(秘匿だけで公開許可が確定するわけではなく、SSJDA と当該調査の条件に従う)。
- 申請経路: 東京大学社会科学研究所附属社会調査・データアーカイブ研究センター(SSJDA)。**調査番号: 【SSJDA 調査番号を SO が転記】**(JLPS-Y/M 2007–、JLPS-2019 の統合提供版)。提供版の識別は統合 Stata ファイル名の末尾番号(`ZQ115…RQ102.dta`; 末尾 `RQ1xx` が w19 の版を示し、差し替えで上がる)と、`results/j1_freeze_record.txt` に記録する SHA-256 による。提供版により有効回答数が僅かに異なりうる(2019 第 1 波: 仲・三輪 2020 は 2,380、本稿の提供版は 2,383)。
- **統合 Stata ファイルから分析用 RDS への変換**は `j1_convert_input.R`(= プロジェクト共通の `paper/jp/jp_combine_wide.R` の写し)が行う。内容は読み込み(`haven::read_dta`)、変数名の小文字化、ラベル付き値の数値化、`cohort`/`entry` 列(統合ファイルでは NA; 初回回答波は `j1_jlps_application.R` が判定)と `source_file` 列の付加、`saveRDS` のみで(複数ファイルを渡したときの縦結合以外に)結合・整形はしない。`Rscript j1_convert_input.R <out.rds> <統合 .dta>`。
...
分析標本: 25–45 歳 → 被用者 → 所得正 → 認可仕様に必要な変数(univ, grades, peduc, married, age, female)が完全(最大標本 4,180)→ 争点 5 変数まで完全(主分析 3,832)。標本フローは `results/j1_flow.csv`。

## 3. 候補グラフと認可対応表 / Candidate graphs and admissibility
...
Rscript j1_convert_input.R <jlps_all_wide.rds> <統合 .dta>        # 公式提供版 → 分析用 RDS(プロジェクト外に保存; 読込・小文字化・数値化・保存のみ)
...
Rscript j1_make_figures.R                                        # results/ の集計値から図 1・図 2 を再描画(個票不要)
Rscript j1_templates.R                                           # §6 雛形 1–3 の役割分類表 → output/j1_templates_output.txt(データ不要)
Rscript j1_freeze_record.R <統合 .dta> <jlps_all_wide.rds> ../results   # 凍結記録: 入力の識別・コードの SHA-256・環境・出力の SHA-256・本文値との照合
```
...
Linux では日本語ラベルのために `LC_ALL=C.UTF-8` を付ける(macOS は不要)。図の PNG は描画フォントに依存し(macOS = Hiragino Sans、Linux = Noto Sans CJK JP)、画素の一致はプラットフォーム内でのみ期待できる。
...
| `j1_specs.csv` | 32 仕様の推定値・HC1 SE・p・認可世界 | 図 2、§7.2 |
| `j1_validity.csv` | 32 仕様 × W1–W5 の認可対応表 | 表 1、§7.1 |
...
| `j1_support.csv` の `main_sample_grades_categorical` 行、`j1_bootstrap.csv` の `*_grades_categorical` 行 | 成績を 4 自由度のカテゴリ変数にした認可 4 仕様と、対応のあるブートストラップ(v4, R3-O1) | 注 3 |
...
- `j1_jlps_application.R` v4(2026-09-17, 知人査読第 3 回): v3 との差は、入力検査、`n_success` の行ごとの定義、成績カテゴリ版の感度と対応のあるブートストラップ。**32 推定値・分解・ブートストラップの定義は変えていない**(合成データで v3 と v4 の `j1_specs.csv`・`j1_flow.csv`・`j1_decomp.txt`・`j1_bootstrap.csv` の共通 61 行がバイト一致することを確認; v3 = `j1_jlps_application_v3_backup.R`)。
- `j1_jlps_application.R` v3(2026-09-17): v2 との差は、認可対応表・区分代表値と中点・働く学生除外・成功反復数と区間方式・図 2 の出力値照合・測定較正量・sessionInfo の追加(v2 = `j1_jlps_application_v2_backup.R`)。
...
- **開示**: `results/` は個票を含まず N < 10 のセルを伏せた集計値だが、**それだけで公開可能とはならない**。公開アーカイブへの収録は SSJDA 利用条件と当該調査の条件に照らした開示確認の後に行う(確認前は `results/`・`figures/` を公開アーカイブから除く; `make_replication_archives.sh` の `J1_INCLUDE_RESULTS=1`)。 **第三者(著者以外の正規利用者)による JLPS 個票での再実行は行われておらず、実データの再現は著者のローカル実行記録(`RUN_LOG_J1.md`、`results/j1_freeze_record.txt`)による。**
- 著者(正式な利用資格者)によるローカル再実行の日時と本文値との照合は `RUN_LOG_J1.md` に記録する。
